...
class WCAUpdater:
    ...
    def _create_database_indexes(self, conn: sqlite3.Connection) -> None:
        ...
        try:
            # 为常用查询字段创建索引（存在才建）
            conn.execute("CREATE INDEX IF NOT EXISTS idx_persons_id ON persons(wca_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_rankssingle_personid ON ranks_single(person_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_rankssingle_eventid ON ranks_single(event_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_ranksaverage_personid ON ranks_average(person_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_ranksaverage_eventid ON ranks_average(event_id)")
    ...
    def process_tsv_to_sqlite(self, tsv_archive_path: Path) -> bool:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # 需要处理的表（按依赖顺序）
                required_tables = [
                    "countries",      # 国家表，用于洲别映射等
                    "events",         # 赛项定义（333、444 等）
                    "competitions",   # 赛事主表，包含比赛时间地点
                    "persons",        # 选手信息
                    "ranks_single",   # 单次排名
                    "ranks_average",  # 平均排名
                ]
