...
- **成绩查询**：查询 WCA 官方成绩(最佳单次/平均)，WR/CR/NR 在小于等于 100/100/200 时显示
...
| 命令 | 描述 | 示例 |
|------|------|------|
| `/wca <WCAID 或姓名>` | 查询个人最佳成绩；若已绑定也可直接 `/wca`；支持 `@已绑定用户` 查询 | `/wca 2026ZHANG01` |
| `/wca绑定 <WCAID 或姓名>` | 将当前 QQ 绑定到指定 WCA 选手 | `/wca绑定 2026ZHANG01` |
| `/wcapic <WCAID 或姓名>` | 生成个人最佳记录图片 | `/wcapic 张伟` |
| `/wcapk <选手1> <选手2>` | PK 对比（WCAID 或唯一姓名） | `/wcapk 张伟 李华` |
| `/one <姓名或ID>` | 查询 one 平台成绩 | `/one 李华` |
| `/pr <姓名>` | 查询 WCA + one 双平台合并 PR；同名可手动指定 WCAID 和 oneID | `/pr 2026ZHANG01 1234` |
| `/prpk <选手1> <选手2>` | 比较两位选手的双平台 成绩；同名可手动指定双方 WCAID 和 oneID | `/prpk 李华 张伟` |
| `/宿敌 <WCAID 或姓名>` | 查询宿敌 | `/宿敌 2026ZHANG01` |
| `/版本` | 查询宿敌后端当前数据库导出日期 | `/版本` |
| `/近期比赛` | 查询近期在中国举办的 WCA 比赛 | `/近期比赛` |
| `/cube帮助` | 显示图片版命令帮助 | `/cube帮助` |
