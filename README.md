...
- 自动下载/更新 WCA TSV → 本地持久化数据 `data/plugin_data/astrbot_plugin_wca/wca_data.db`，支持定时（默认 12 小时检查一次）
- WCA官方成绩查询：最佳单次/平均，WR/CR/NR（前 200 显示）
- PK 对比：两位选手单次与平均逐项比较，标注优势项 (☆/★)，统计总分 (⭐)
- 宿敌查询：统计在共同项目中“单次与平均均优于你”的选手数量（世界/洲/地区），人数少于等于5时会列出名单
