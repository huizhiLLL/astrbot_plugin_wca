...
WCA 与 one 成绩查询插件。全 API 架构，实时查询 WCA 官方数据与 one 平台数据。
...
- `enable_command_reaction`：是否启用命令接收后的表情反馈，默认开启
- `command_reaction_emoji_id`：QQ 使用的贴表情 `emoji_id`，默认 `181`
-  `/cube帮助`、`/wcapic`、`/wcapk`、`/onepk`、`/pktwo`、`/pr`、`/prpk`、`/宿敌`、`/版本`、`/近期比赛` 会使用这套即时反馈
