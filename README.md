...
- `command_reaction_emoji_id`：QQ / aiocqhttp 使用的贴表情 `emoji_id`，默认 `181`
- 当前 `/cube帮助`、`/wcapic`、`/wcapk`、`/onepk`、`/pktwo`、`/pr`、`/prpk`、`/宿敌`、`/版本`、`/近期比赛` 会使用这套即时反馈
...
- `/one绑定 <oneID或用户名>` 会把当前 QQ 绑定到对应 one 用户
- 绑定成功后，直接发送 `/wca`、`/wcapic`、`/宿敌`、`/one` 即可查询自己的相关信息
- 发送 `/wca @某人`、`/wcapic @某人`、`/宿敌 @某人` 时，如果对方已绑定 WCAID，也可以直接查询对方信息
...
- 发送 `/prpk @某人` 时，会自动使用“自己 vs 对方”的两边绑定；发送 `/prpk @甲 @乙` 时，会直接比较这两个用户的双平台 PR
