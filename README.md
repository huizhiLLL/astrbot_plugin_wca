...
- **成绩查询**：WCA 官方成绩查询，最佳单次/平均，WR/CR/NR 在小于等于 100/100/200 时显示
- **WCA 绑定**：支持将当前 QQ 与 WCAID 绑定，绑定后可直接使用 `/wca` 查询自己，也可通过 `@` 已绑定用户快速查询
- **成绩图片**：生成个人最佳记录图片卡片（仿 WCA 官网），支持 WCAID 或姓名查询
- **PK 对比**：两位选手单次与平均逐项比较，标注优势项 (☆/★)，统计总分 (⭐)
- **宿敌查询**：通过外部 API 查询宿敌，统计在共同项目中"单次与平均均优于你"的选手数量（世界/同洲/同国家或地区），人数少于等于 10 时会列出名单
- **版本查询**：查询宿敌后端当前数据库导出日期，便于确认数据更新时间
...
| 命令 | 描述 | 示例 |
|------|------|------|
| `/wca <WCA ID 或姓名>` | 查询个人最佳成绩；若已绑定也可直接 `/wca`；支持 `@已绑定用户` 查询 | `/wca 2026ZHANG01` |
| `/wca绑定 <WCA ID 或姓名>` | 将当前 QQ 绑定到指定 WCA 选手 | `/wca绑定 2026ZHANG01` |
| `/wcapic <WCA ID 或姓名>` | 生成个人最佳记录图片 | `/wcapic 张伟` |
| `/wcapk <选手1> <选手2>` | PK 对比（WCA ID 或唯一姓名） | `/wcapk 张伟 李华` |
| `/宿敌 <WCA ID 或姓名>` | 查询宿敌 | `/宿敌 2026ZHANG01` |
...
- 宿敌查询适配新的后端接口返回语义，展示文案改为世界 / 同洲 / 同国家或地区
